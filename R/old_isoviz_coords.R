...
isoviz_coords = function(file_path, gene_trans,
  ...
  if(input_type=="psl"){
    ...
    trans_info = genome_data %>% dplyr::select(1, 4, 5, 6) %>% distinct() %>% dplyr::filter(chr != "chrY", chr != "chrM")
    intron_data = intron_data %>% left_join(trans_info,  by = "trans_id") %>%
      left_join(convert, by = c("gene_id", "trans_id")) %>% dplyr::select(4, 2, 3, 5, 1, 6, 7, 8, 9, 10)
