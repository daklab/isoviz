...
plot_all_junction_to_isoform = function(gene = "ENSG00000001167.15", cell_type = "hESC", junction_usage = 5, include_common_juncs = FALSE, filter_expressed_isoforms = TRUE){
  ...
  if(filter_expressed_isoforms == TRUE){
    remove_junc = introns %>% filter(junc.usage & isoforms %in% expressed_isoforms$isoforms)
...
get_guide_predictions = function(junc_list, cell_type = "hESC", guides_per_junction = 2){
  ...
  output = top_guides %>% left_join(all_gene_clusters) %>%
    select(Cluster = cluster_idx, JuncID = junc_id, Category = junction_category, Counts = junc.counts, Usage = junc.usage,
           Isoforms = transcript_isoforms, GuideID = guide_id, GuideSeq = guide_sequence, Prediction = predicted_lfc)
