...
# total reads in a375-cyt_Aligned.q20.sort.bam: 43,407,715
...
a375_screen_lfc_clusters = as.data.table(clusters)
save(a375_screen_lfc_clusters, file = "~/isoviz/data/a375_screen_lfc_clusters.rda")
...
map_junction = function(gene = "ENSG00000001167.15", cell_type = "A375"){
  ...
  const_junc_list = a375_screen_lfc_clusters %>% 
    group_by(cluster.n) %>% 
    tally() %>% filter(n == 1) %>% ungroup()
  a375_screen_lfc_clusters %<>% mutate(junc.type = ifelse(cluster.n %in% const_junc_list$cluster.n, "Common", "Unique"))
  gene_cluster = gene_iso_data %>% 
    left_join(a375_screen_lfc_clusters, by = c("chr", "intron_starts" = "start", "intron_ends" = "end")) %>%
  ...
  output = a375_screen_lfc_clusters %>% filter(cluster.n %in% cluster_ids) %>% group_by(cluster.n) %>% mutate(junc.per.cluster = n()) %>% 
